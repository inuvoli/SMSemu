...
Cartridge::Cartridge(const std::string& sFileName, uint8_t nMapper)
{
	uint32_t romsize;
	...
	switch (nMapper)
	{
	case SEGA:			pMapper = std::make_shared<MapperSega>(cROM, cRAM); break;
	case CODEMASTER:	pMapper = std::make_shared<MapperCodemaster>(cROM, cRAM); break;
	}
}
